...
void iftDestroyImage(iftImage **img) 
{
    if(img != NULL) {
        ...
        if (aux != NULL) {
            ...
            if (aux->R != NULL) iftFree(aux->R);
            if (aux->G != NULL) iftFree(aux->G);
            if (aux->B != NULL) iftFree(aux->B);
    ...
    switch (cinfo.out_color_space){
        ...
        case JCS_RGB:
            ...
            while (cinfo.output_scanline < cinfo.output_height){
                ...
                for (unsigned int i = 0; i < (unsigned int)cinfo.output_width; i++) {
                    ...
                    iftImgR(image,imageCol,imageRow,0) = rgb.val[0];
                    iftImgG(image,imageCol,imageRow,0) = rgb.val[1];
                    iftImgB(image,imageCol,imageRow,0) = rgb.val[2];
                    imageCol++;
                }
                imageRow++;
            }
        ...
        case JCS_YCbCr:
            ...
            while (cinfo.output_scanline < cinfo.output_height){
                ...
                for (unsigned int i = 0; i < (unsigned int)cinfo.output_width; i++) {
                    int shift = i*cinfo.num_components;
                    YCbCr.val[0] = buffer[0][shift+0];
                    YCbCr.val[1] = buffer[0][shift+1];
                    YCbCr.val[2] = buffer[0][shift+2];
...
iftImage *iftReadImageP6(const char *format, ...)
{
    ...
    if(iftCompareStrings(type,"P6")){
        ...
        zsize = 1;
        img = iftCreateImage(xsize,ysize,zsize);
        img->R = iftAllocIntArray(img->n);
        img->G = iftAllocIntArray(img->n);
        img->B = iftAllocIntArray(img->n);
        img->dz = 0.0;
        if (fscanf(fp,"%d",&v)!=1)
            iftError("Reading error", "iftReadImageP6");
...
*/
    ...
    while (cinfo.next_scanline < cinfo.image_height) {
        ...
        for (unsigned int i = 0; i < (unsigned int)cinfo.image_width; i++) {
            int shift = i*cinfo.num_components;
            buffer[0][(shift+0)] = (unsigned char)iftImgR(img,imageCol,imageRow,0);
            buffer[0][(shift+1)] = (unsigned char)iftImgG(img,imageCol,imageRow,0);
            buffer[0][(shift+2)] = (unsigned char)iftImgB(img,imageCol,imageRow,0);
...
void    iftSetRGB(iftImage *img, int value)
{
    int p;
